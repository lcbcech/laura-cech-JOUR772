...
#creating an empty list to write the output of our function to
abbrev_state_names <- c()

#using a for loop to cycle through each of the names in our full_state_names list
for (name in full_state_names){
  
  #handling dc, the function doesnt do it
  if(name == "district of columbia"){
    
    #setting abbreviation for dc manually
    abbrev_state_name == "DC"
  
  #letting the function get the abbreviation for every other state
  } else {
    
     #use function to create the abbreviation
    abbrev_state_name <- full_to_abbrev(name) 
    
  }
  
  #add that abbreviation to the blank list we created before the for loop
  abbrev_state_names <- c(abbrev_state_names, abbrev_state_name)
...
```{r}
public_21 <- public |>
  filter(st %in% abbrev_state_name) |>
