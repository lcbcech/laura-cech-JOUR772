...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** It appears not.
...
vendor is character type, amount is number, date is a character 
...
**Answer** There are multiple "na" in the category type. 
...
**Answer** Some zip codes have 5 digits, some have a hyphen and an extra four digits; some grantees are in all-caps, others are title case. Two grants are less than $100, which seems low for the grant process.
...
**Answer**  clean names allows you to change header names, changing name of source column to grantor, mutating the capitalization of source, grantee and description columms to uppercase.
...
**Answer** It created a new zip5 column with 5-digit zip codes
...
**Answer** Zipcode with the most amount is 5,094,146,889.61 in Washington, DC. I guess it makes sense since Maryland is trying to get tourists who come to the area for DC primarily, but I'm a little surprised that it's that much. I would've thought the most money would be spent on advertising.
