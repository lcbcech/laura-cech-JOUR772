...
```{r}
#Disclaimer: asked chatgpt for help dealing with the geoid and zipcode difference.

pgco_911_calls <- mutate(pgco_911_calls, zipcode = as.character(zipcode))

# Perform the join
calls_with_census_data <- left_join(pgco_911_calls, pg_census, by = c("zipcode" = "GEOID"))

# To see the result
glimpse(calls_with_census_data)
```
```{r}
#looked up scatterplot
ggplot(calls_with_census_data, aes(x = estimate, y = number_of_calls)) +
  geom_point() +
  labs(x = "Median Income", y = "Total 911 Calls", title = "Scatterplot of 911 Calls vs Median Income") +
  theme_minimal()
```

```{r}
#asked chatgpt for help dealing with error
calls_with_census_data <- pgco_911_calls %>%
  group_by(zipcode) %>%
  summarise(number_of_calls = n()) %>%
  left_join(pg_census, by = c("zipcode" = "GEOID"))

# Then, retry plotting with the correct column name:
ggplot(calls_with_census_data, aes(x = estimate, y = number_of_calls)) +
  geom_point() +
  labs(x = "Median Income", y = "Total 911 Calls", title = "Scatterplot of 911 Calls vs Median Income") +
  theme_minimal()
...
calls_with_census_data <- na.omit(calls_with_census_data)
