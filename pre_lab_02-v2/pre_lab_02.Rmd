...
**Answer** Whatever actions we take now, will be for that newly created subset.
...
**Answer:** dates are now values, rather than characters. 
...
**Answer** Organized by year, month, day. A possible glitch of some months having one number and others having two seems fine because there's a zero placeholder
...
**Answer** revising general_22 to include new columns of inforamtion
...
How did the answer change from the previous task, and why? Yes, now in descending order
...
**Answer** It's interesting to see top and bottom, places where he has support and where he doesn't.
...
what are some tidyverse functions similar to str_to_upper?
what's a shorter way?
library(dplyr)
...
# Apply str_to_upper to all character columns
data <- data %>%
  mutate(across(where(is.character), str_to_upper))

print(data)

In this example, mutate is used with across to apply the str_to_upper function to all character columns in the data frame data. This approach is shorter and more flexible when you need to apply the same transformation to multiple columns.
...
# Apply str_to_upper to all character columns using mutate_all
data <- data %>%
  mutate_all(str_to_upper)

# Alternatively, you can specify columns by name using mutate_at
# data <- data %>%
#   mutate_at(vars(text1, text2), str_to_upper)

print(data)

In this example, mutate_all applies the str_to_upper function to all columns in the data frame, and mutate_at allows you to specify columns by name or conditionally based on their properties. Both approaches are shorter and more efficient when you want to apply the same transformation to multiple columns in your data frame.
**Answer**
...
**Answer** In cases where you detect "Maryland" or something like it, call it "MD" and if not any of those, default to NA. Curious about why we have spaces before Maryland in quotes and if that will be resolved by the ~ 
...
Does this answer make sense to you? Why or why not? I think it's showing that 7,772,3145.82 was spent in MD, but there's a lot in NA so that could significantly change things
...
1. I'd like to figure what's happening with NA, so it would be good to figure out how to detect for the multiple ways that DC could be entered.

2.  How are each of the candidates spending in each state?

3.  Are any of the candidates paying themselves or family members?
