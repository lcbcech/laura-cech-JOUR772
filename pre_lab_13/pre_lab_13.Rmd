...
**Answer**  5 columns of data, including per pupil spending for 2021, gross domestic product growth for 2021, population change for 2021 and winner in 2020 election for 50 rows (one for each )
...
**Answer** There seems to be a lot of gdp growth
...
**Answer** spending more on students doesn't seem to result in higher gdp.
```{r}
...
**Answer** Population change doesn't seem impact gdp grwoth or per pupil spending much either.
...
**Answer** the p-value is less than 0.05, which is is judged as “significant,” 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** the p-value is more than 0.05, which means there's a higher probability that the difference is due to chance
```{r}
...
**Answer** States that voted for Biden had higher gdp growht, per pupil spending and smaller population growth than those that voted for Trump
...
**Answer** more likely accurate than just chance.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** difference more likely to be accurate
