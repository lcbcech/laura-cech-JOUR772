...
**Task** Run the following code to filter on Frederick County's registration data and look at the results. What stands out to you about the changes in party registration over time? **Answer** The growth of Democrats is substantial (about 28%) 
 versus Republicans (only about 5%). Although relatively small, there are also 4 times as many as voters who identify themselves as "other" from 2016 to 2022.
```{r}
...
**Task** Run the following code to join Maryland population data and the 2022 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **Answer** 24 rows. Since there are 23 counties and Baltimore City, there should be 24.
...
**Task** Load the Maryland demographic data and examine it. What kind of information does it contain? **Answer** 24 rows (all counties and city) with 8 columns of population data (10 columns total)
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Answer** The highest 18+ Black populations are in Prince George's County and Baltimore City, where Democrats make up more than 75% of voters. Nearly half of the population in Charles County is Black, with 61 percent Democrats, and I wonder if those numbers have been shifting in recent years. The population of Somerset County on the Eastern shore is nearly 40 percent Black with a similar percent of Democrats. I wonder if we could see the percentage of Dems/ Republicians by race.
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Answer** Howard and Montgomery counties have the most Asian populations where Democrats make up 53 and 61 percent respectively. But there are less than 5% Asian population in Prince George's County and Baltimore where the highest Black populations are.
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N column? **Answer** It's a number.
