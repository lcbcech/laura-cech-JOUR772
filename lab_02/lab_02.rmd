...
**Q1.** How many departments have offered the courses with "Climate" in the title and at least one seat? Which department has offered the most? **A1.*69*
...
```{r}
and_climate_seats_1 <- umd_courses |> filter(department == "climate" | seats >= 1)|>
count()|>
arrange(desc(n))
```

**Q2.** Of the departments listed in A1, which one is the most interesting to you, and why? 
**A2.*Economics, because it's not sceince-related.*
...
courses_2023 <- umd_courses |> filter(term)
