...
cleanedup_payments <- read_csv("data/maryland_expenses.csv") |>
  clean_names() 
```
```{r}

  glimpse(cleanedup_payments)

```
```{r}
cleanedup_payments |> mutate(expenditure_date=mdy(expenditure_date)) 
```
...
```{r}
write_csv(cleaned_payments, "cleanedup_payments.csv")

```


```{r}
cleanedup_payments2 <- read_csv("data/cleanedup-payments2-csv.csv") 
```
...
**A2.** The top spenders are campagin accounts for high profile candidates such as Wes Moore for Govenor, David Blair for Montgomery County Executive, Moore/Miller for Gov./ Lt. Gov., Comptroller Peter Franchot. The facebook ads seem to be paid for by campaign accounts and are inactive.
